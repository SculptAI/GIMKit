...
def build_cfg(query: Query) -> CFG:
    ...
    The grammar field in MaskedTag allows users to provide custom LLGuidance CFG grammar
    patterns that are embedded directly into the CFG. This provides flexibility but requires 
    users to ensure their grammar is valid syntax and compatible with the surrounding CFG 
    structure. Invalid grammar will cause parsing errors when the CFG is used for generation.
    """
